...
<%out.print(FrontScreen.rackName(start_epoch, end_epoch));%>
	...
	function displayServerInfo(element, number) {
		document.getElementById("information").innerHTML = "<p>Server Number: "
				+ parseInt(hostID[number]) + "</p><p>Max Power: "
				+ hostMax[number] + "</p><p>Average: " + hostAverage[number]
				+ "</p>";
		var elements = document.getElementsByName("server"
				.concat(current_host + 1));
...
<h2>Below show's all Racks, inside each rack show's their server. The box on the left will display any server data once clicked.</h2>
	<div id='serverInfo'>
